...
namespace Beat {
SynthesizerComponent::SynthesizerComponent(logger::Logger& l)
    : logger(l) {
    up.onClick = [this]() {
    ...
    for (int i = 0; i < totalMultipleCount; ++i) {
        ...
        button.onClick = [this, i]() { setNextMultipleIndex(i); };
        juce::Path rect;
...
void SynthesizerComponent::setMultipleFromIndex(int m) {
    multipleButtons[multipleIndex].setColours(
...
void SynthesizerComponent::resized() {
    ...
    for (int i = 0; i < totalMultipleCount; ++i) {
        multipleButtons[i].setBounds(
            rect.removeFromLeft(rect.getWidth() / (totalMultipleCount - i)));
    }
}
